...
if __name__ == "__main__":
    ...
    directories = ['src', 'test', 'include', 'scripts', 'assets/sql']
    ...
    for directory in directories:
        for root, subFolders, files in os.walk(mypath + directory):
            for fileEntry in files:
                if (fileEntry.endswith('.h') or fileEntry.endswith('.cpp') or fileEntry.endswith('.py') or fileEntry.endswith('.cl') or fileEntry.endswith('.sql')):
    ...
    f.write('Personal Finance Tracker 2019 Build Information\n')
